...
# pattern rule が遅そうなので for 文に書き換えた
$(ALL_LSTMF):
	$(file >$@)
	$(eval NUM_PNG := $(shell find $(GROUND_TRUTH_DIR) -name "*.png" | wc -l))

	@i=0;\
	for png in $(GROUND_TRUTH_DIR)/*.png; do\
		if [ ! -f $${png%%.png}.lstmf ]; then\
			PYTHONIOENCODING=utf-8 python3 $(GENERATE_BOX_SCRIPT) -i $$png -t $${png%%.png}.gt.txt > $${png%%.png}.box;\
			tesseract $$png $${png%%.png} --psm $(PSM) lstm.train;\
		fi;\
		i=`expr $$i + 1`;\
		echo $$i/$(NUM_PNG) $${png%%.png}.lstmf;\
		echo $${png%%.png}.lstmf >> $@;\
	done
